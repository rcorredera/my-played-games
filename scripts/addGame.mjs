...
async function fetchGameFromIGDB(gameId, clientId, accessToken) {
  console.log(`Tentative de récupération des informations pour le jeu ID: ${gameId}`);
  try {
    ...
    if (game) {
      ...
      if (!coverResponse.ok) {
        console.error(`Erreur HTTP: ${coverResponse.status} lors de la récupération de la couverture du jeu.`);
        return null;
      }
...
async function main() {
  ...
  if (!clientId || !accessToken || !gameId) {
    console.error('Veuillez fournir le CLIENT_ID, le ACCESS_TOKEN, et l\'identifiant de jeu.');
    console.error('Usage: node addGame.mjs <CLIENT_ID> <ACCESS_TOKEN> <GAME_ID>');
    return;
  }
