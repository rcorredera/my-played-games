...
const platformIconsFilePath = path.join(path.dirname(fileURLToPath(import.meta.url)), '../src/data/platforms.json');
...
async function fetchAllPlatformsFromIGDB(clientId, accessToken) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`Erreur HTTP: ${response.status} lors de la récupération des plateformes.`);
      return [];
    }
    ...
    console.error('Erreur lors de la récupération des données des plateformes:', error);
    return [];
  }
}
...
async function fetchPlatformLogos(platforms, clientId, accessToken) {
  const logoIds = platforms.map(platform => platform.platform_logo).filter(Boolean);
  if (!logoIds.length) {
  ...
  try {
    for (let i = 0; i < logoIds.length; i ++) {
      ...
      if (!response.ok) {
        console.error(`Erreur HTTP: ${response.status} lors de la récupération des logos de plateforme pour le lot ${i / batchSize + 1}.`);
        continue;
      }

      const logo = (await response.json())[0];
      logoMap[logo.id] = { url: `https:${logo.url}`, image_id: logo.image_id };
      console.log(`URL du logo de plateforme récupérée pour l'ID ${logo.id}: ${logoMap[logo.id].url}`);
    }
    ...
    console.error('Erreur lors de la récupération des données des logos de plateforme:', error);
    return [];
  }
}
...
async function appendPlatformsToFile(platforms) {
  ...
  try {
    ...
    platforms.forEach(platform => {
      const platformExists = existingPlatforms.some(existingPlatform => existingPlatform.id === platform.id);
      if (!platformExists) {
        ...
        console.log('La plateforme est déjà présente dans la liste:', platform.name);
      }
    });

    console.log('Contenu de existingPlatforms avant la sérialisation:', JSON.stringify(existingPlatforms, null, 2));
    ...
    console.error('Erreur lors de la manipulation du fichier platforms.json:', err);
  }
}
...
async function main() {
  ...
  const platformsWithLogos = await fetchPlatformLogos(platforms, clientId, accessToken);
